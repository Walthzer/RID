...
if (isServer) then {
    if (is3DEN) then {
        ...
        ["RID Initialize Eden"] collect3DENHistory {
            ...
            //For "on3DENStart" Modules call module function.
            private _allModules = all3DENEntities select 3;
            {
                private _moduleConfig = configOf _x;
                if ((getNumber (_moduleConfig >> QGVARMAIN(on3DENStart))) isEqualTo 1) then {
                    ...
                    ["edenStart", [_x]] call _moduleFunction;
                };
            } forEach _allModules;
        };
