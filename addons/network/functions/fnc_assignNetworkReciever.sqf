...
/*
 * Turn a Network Node into a Network Reciever. Storing a function to be called when a Network Crawler processes the node.
 *
 * Arguments:
...
if(_arguments isEqualTo []) exitWith {_node setVariable[QGVAR(NetworkReciever), [_function], true]};
_node setVariable[QGVAR(NetworkReciever), [_function, _arguments], true];