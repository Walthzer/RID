...
class Segment():
    ...
    def calculate_bbox_polygon(self, time):
        ...
        dialog.printText.emit("Segment <{}>...".format(self.id), 0, False)
        ...
        while z < 2: 
            ...
            z += 1
        self.polygon = CCPolygon(self.polygon_points)
        dialog.printTextPlain.emit("Completed.")

    def arma(self):
        return [self.vertex0.arma(), self.vertex1.arma(), self.trace.time]

class Trace():
    def __init__(self, contour=None, id_=0, polygon_fidelity=5):
        ...
        dialog.printText.emit("Approximating polygon...", 1, False)
        self.polygon = self.create_polygon_from_contour(contour)
        dialog.printTextPlain.emit("Completed.")

        dialog.printText.emit("Calculate Skeleton...", 0, True)
        sg_skeleton = skeleton.create_interior_straight_skeleton(self.polygon)
        dialog.printText.emit("interior skeleton obtained.", 1, True)

        self.merge_groups = self.create_merge_groups(sg_skeleton)
        dialog.printText.emit("Skeleton reduced.", 0, True)

        dialog.printText.emit("Skeleton calculations complete.", -1, True)

        self.polygon_fidelity = polygon_fidelity + 1

        dialog.printText.emit("Calculate Segments...", 0, True)
        self.segments, self.time = self.create_segments(sg_skeleton, self.merge_groups)
        
        dialog.printText.emit("Obtain segments bounding box polygons...", 1, True)
        dialog.changeIndentSignal.emit(1)
        for seg in self.segments:
            seg.calculate_bbox_polygon(self.time)
        dialog.printText.emit("Segment calculations complete.", -2, True)
    def __getstate__(self):
        state = self.__dict__.copy()
...
class Sectors:
    def __init__(self, res=Resolution(0,0), shape=Resolution(3,3)):
        self.shape = shape
    ...
    def calculate_sectors(self, res, shape):
        ...
        while iy < shape.y:
            ...
            while ix < shape.x:
                ...
                sectors_list[sector_name] = Sector(sector_name, Resolution(x0, y0), Resolution(x1, y1))
                ix += 1
            iy += 1
...
class Sector():
    ...
    def intersections_with_trace(self, trace):
        if not self.do_intersect(trace.polygon):
            return False
